...
¡Nos vemos en el curso!`;
  ...
  // Endpoint para simular callback (solo para testing)
  ...
  //   @Res() res: Response
  // ) {
  //   this.logger.log(`🧪 SIMULANDO CALLBACK para: ${clientTransactionId} con status: ${status}`);
  ...
  //       amount: paymentAttempt.amount,
  //       authorizationCode: 'SIM123456',
  //       message: 'Transacción simulada exitosamente'
  //     };

  //     await this.paymentAttemptRepo.update(paymentAttempt.id, {
  //       payphoneId: 'SIMULATED_PAYMENT_ID',
  //       status: status,
  //       callbackData: JSON.stringify(mockConfirmData),
  //       updatedAt: new Date()
  //     });
  ...
  //       return res.redirect(`${frontendUrl}/pago-exitoso?clientTransactionId=${clientTransactionId}`);
  //     } else {
  //       return res.redirect(`${frontendUrl}/pago-fallido?clientTransactionId=${clientTransactionId}&status=${status}`);
  //     }

  //   } catch (error) {
  //     this.logger.error(`💥 Error en simulación:`, error);
  //     return res.redirect(`${frontendUrl}/pago-fallido?error=error_simulacion`);
  //   }
  // }
}